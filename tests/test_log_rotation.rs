...
#[cfg(test)]
mod tests {
    use rlg::config::{LogRotation, ConfigError};
    use std::str::FromStr;
    ...
    fn log_rotation_from_str_valid_inputs() {
        let test_cases = vec![
            ("size:1024", LogRotation::Size(NonZeroU64::new(1024).unwrap())),
            ("time:60", LogRotation::Time(NonZeroU64::new(60).unwrap())),
            ("date", LogRotation::Date),
            ("count:5", LogRotation::Count(5)),
            ("SIZE:512", LogRotation::Size(NonZeroU64::new(512).unwrap())),
        ];
    ...
    fn log_rotation_from_str_invalid_inputs() {
        ...
        for (input, expected_err) in test_cases {
            ...
            match result {
                Err(ConfigError::ValidationError(msg)) => assert_eq!(msg, expected_err, "Failed for input: {}", input),
                _ => panic!("Expected ValidationError for input: {}", input),
            }
        }
    }
    ...
    fn log_rotation_display() {
        assert_eq!(format!("{}", LogRotation::Date), "Date-based rotation");
        assert_eq!(format!("{}", LogRotation::Size(NonZeroU64::new(100).unwrap())), "Size: 100 bytes");
        assert_eq!(format!("{}", LogRotation::Time(NonZeroU64::new(50).unwrap())), "Time: 50 seconds");
        assert_eq!(format!("{}", LogRotation::Count(10)), "Count: 10 logs");
    }
}
