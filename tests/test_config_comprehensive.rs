...
#[cfg(test)]
mod tests {
    use rlg::config::{Config, LoggingDestination, ConfigError};
    ...
    fn test_validate_network_address_is_ignored() {
        let config = Config {
            logging_destinations: vec![LoggingDestination::Network("nonexistent.invalid:8080".to_string())],
            ..Config::default()
        };
    ...
    fn test_config_error_all_variants() {
        ...
        assert_eq!(format!("{}", err), "Configuration validation error: val");
        
        let err = ConfigError::FileWriteError("io".to_string());
        assert_eq!(format!("{}", err), "File write error: io");

        let err = ConfigError::EnvVarParseError(envy::Error::Custom("env".to_string()));
        assert_eq!(format!("{}", err), "Environment variable parse error: env");
        ...
        assert!(format!("{}", err).contains("Configuration parsing error:"));
    }

    #[test]
    fn test_logging_destination_display() {
        let dest = LoggingDestination::Network("127.0.0.1:8080".to_string());
        assert_eq!(format!("{:?}", dest), "Network(\"127.0.0.1:8080\")");
    ...
    async fn test_config_validate_create_parent_dir_fail() {
        ...
        let config = Config {
            logging_destinations: vec![LoggingDestination::File(log_file)],
            ..Config::default()
        };
